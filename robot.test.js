...
describe('Movement and reporting', () => {
  ...
  beforeEach(() => {
    initialState = {
      x: 0, y: 0, facing: Facing.NORTH, isPlaced: true,
    };
  });

  it('should move when the move command is given', () => {
    const expectedState = {
      x: 0, y: 1, facing: Facing.NORTH, isPlaced: true,
    };
  ...
  it("should turn left when the 'left' command is given", () => {
    const expectedState = {
      x: 0, y: 0, facing: Facing.WEST, isPlaced: true,
    };
  ...
  it("should turn right when the 'right' command is given", () => {
    const expectedState = {
      x: 0, y: 0, facing: Facing.EAST, isPlaced: true,
    };
  ...
  it('should move in the direction that it is facing after turning', () => {
    ...
    const expectedState = {
      x: 1, y: 0, facing: Facing.SOUTH, isPlaced: true,
    };
...
describe('Robot placement', () => {
  ...
  it("should get placed in the location specified by the 'place' command", () => {
    ...
    const expected = {
      x: 1, y: 1, facing: Facing.EAST, isPlaced: true,
    };
