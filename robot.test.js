...
const {Actions, Facing} = require('./constants');
...
describe("Movement and reporting", () => {
    ...
    beforeEach(() => {
        initialState = { x: 0, y: 0, facing: Facing.NORTH, isPlaced: true };
    });

    it("should move when the move command is given", () => {
        const expectedState = { x: 0, y: 1, facing: Facing.NORTH, isPlaced: true };
    ...
    it("should turn left when the 'left' command is given", () => {
        const expectedState = { x: 0, y: 0, facing: Facing.WEST, isPlaced: true };
    ...
    it("should turn right when the 'right' command is given", () => {
        const expectedState = { x: 0, y: 0, facing: Facing.EAST, isPlaced: true };
    ...
    it("should move in the direction that it is facing after turning", () => {
        ...
        const expectedState = { x: 1, y: 0, facing: Facing.SOUTH, isPlaced: true };
        expect(actualState).toEqual(expectedState);
    });

    it("should not allow the robot to move out of the map bounds", () => {
        initialState = { x: 4, y: 4, facing: Facing.NORTH };
...
describe("Robot placement", () => {
    ...
    it("should get placed in the location specified by the 'place' command", () => {
        const data = { x: 1, y: 1, facing: Facing.EAST};
        const expected = { x: 1, y: 1, facing: Facing.EAST, isPlaced: true};
    ...
    it("should ignore place commands that attempt to place the robot in an invalid location", () => {
        const data = { x: 6, y: 6, facing: Facing.EAST};
...
describe("Robot input processing", () => {
    ...
    it("should convert a valid input string into a command", () => {
        expect(robot.processInput("move")).toEqual([{action: Actions.MOVE}]);
    });
    ...
    it("should convert multiple space seperated input actions into multiple commands", () => {
        const expected  = [{action: Actions.PLACE, data: {x: 2, y: 3, facing: Facing.NORTH}},
                            {action: Actions.LEFT},
                            {action: Actions.MOVE}];
    ...
    it ("should normalise the actions before returning them", () => {
        expect(robot.processInput("MoVe")[0].action).toEqual(Actions.MOVE);
        expect(robot.processInput("  MoVe       ")[0].action).toEqual(Actions.MOVE);
    })
